...
logging.basicConfig(
    ...
    filename="C://Users/cstan/Documents/CooperStandard/tutorials/EUAIops/logs/GraphSAGELog0922323.log",
...
logging.debug(f"Feature Keys: {feature_keys}")
...
logging.debug(f"All_node_features: {all_node_features}")
...
for node in range(graphDGL.number_of_nodes()):
    ...
    for key in feature_keys:
        ...
        try:
            ...
            logging.debug(f"Trying to mask a 0-d tensor.")
        except IndexError:
            print(
...
#        return torch.tensor(data_dict, dtype=torch.float32)
    try:
        ...
        logging.critical(f"Could not find items in data_dict term")
    except Exception as e:
        logging.critical(f"dict_to_tensor exception")
    try: 
        if(np.array(data_dict).any() and tensors== []):
            array = np.zeros((data_dict.size(0)), dtype=np.float32)
            return torch.tensor(array, dtype=torch.float32)
        return torch.cat(tensors)
    except Exception as e:
        logging.critical("Dict_to_Tensor return torch.cat(tensors) failed")
...
class GraphSAGE:
    ...
    def aggregate_neighbors(self, node, depth=2):
        ...
        for jj in range(ii):
            #            neighborhood_tensors[jj] = [torch.tensor(value) for value in features_dict[jj].values()]
            #            neighborhood_tensors[jj] = dict_to_tensor(features_dict[jj])
            #            if(features_dict[jj].dtype==torch.float32):
            #                neighborhood_tensors_list.append(features_dict[jj])
            #            else:
    ...
    def generate_embedding(self, node):
        ...
        try:
            ...
            logging.critical(f"aggregated_feat is length 1, type float, value NAN")
    ...
    def train_reducer(self, num_epochs=10):
        for epoch in range(num_epochs):
            ...
            for node in enumerate(self.features):
                concatenated_embedding = self.generate_embedding(node)
...
embeddings = {index: graphsage.generate_embedding(node_features) for index, node_features in enumerate(nodes_feature_list)}
print(embeddings)
...
logging.CRITICAL(f"{similarity_matrix}")
...
logging.debug(f"sorted_node_pairs:  {sorted_node_pairs}")
