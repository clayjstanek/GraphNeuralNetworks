...
import joblib
import json
from camgian_python_shared_base.postgres_utilities.context_manager import PostgresContextManager
from mapped_classes.association_document import association_document
from mapped_classes.association_entity import association_entity
from mapped_classes.association_entity_document_bridge import association_entity_document_bridge
from mapped_classes.association_entity_entity import association_entity_entity
from mapped_classes.association_entity_fact import association_entity_fact
from mapped_classes.association_entity_topic_bridge import association_entity_topic_bridge
from mapped_classes.association_timestamp import association_timestamp
from mapped_classes.association_topic import association_topic
from mapped_classes.association_topic_document_bridge import association_topic_document_bridge
from mapped_classes.spatial_activity import spatial_activity

from sqlalchemy import event, text, and_, select, inspect, update, insert, or_, func
...
def create_graph(db_credentials, start_time, end_time):
    ...
    feature_keys = [key for key in nodes_feature_list[0].keys()
                    if (key != 'node_name' and key != 'node_id')]
...
class GraphSAGEAutoencoder(nn.Module):
    ...
    def aggregate_neighbors(self, node, depth=2):
        ...
        print(neighborhood_tensors_list)
...
def run_group_formation_module(db_credentials, start_time, end_time):
    ...
    graphsage = GraphSAGEAutoencoder(adj_list, nodes_feature_list, feature_keys,
                                     input_dim=NODELEN*2, embedding_dim=128, hidden_dim1=300,
