...
def create_graph(db_credentials, start_time, end_time):
    ...
    end_time = '2023-08-21 20:54:06.000'
...
#        yaml.dump(metadata_dict, file)
    ...
    nodes_df = pd.read_pickle('./new_nodes_df.pkl')
    edges_df = pd.read_pickle('./new_edges_df.pkl')
        ...
        try:
            ...
            try:
                if data_dict.dtype == torch.float32:
                    tensors = data_dict
                    logger.info("----Returning tensors that was a torch.float32")
                    return tensors
            except Exception as e:
                logger.info("----Returning tensors that is NODELEN of zeros torch.float32")
...
class GraphSAGEAutoencoder(nn.Module):
    ...
    def generate_embedding(self, node, TRAIN=False):
        ...
        embedding_tensor[(NODELEN - 1)*2] = 0.0
