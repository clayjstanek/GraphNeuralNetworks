...
NODELEN = 15 # 393 # expected node len for this particular batch of data
BATCHSIZE = 32
NUM_EPOCHS = 100
...
def create_graph(db_credentials, start_time, end_time):
    ...
    # graph_sample = random.sample(list(init_graph.nodes()), 500)
    # graph = init_graph.subgraph(graph_sample)
    ...
    # print(nodes_df)
    # print("transformer output: " + str(label_embeds))
    # print("transformer output size: " + str(len(label_embeds)) + " x " + str(len(label_embeds[0])))
        ...
        for feature in all_features:
            if feature != 'label' and feature != 'node_id':
                if feature == 'label_embeds':
                    feature_dict[node['node_id']][feature] = torch.tensor(node[feature])
                else:
                    feature_dict[node['node_id']][feature] = node[feature]
        label_dict[node['node_id']] = node['label']
    # print("features applied to first node: " + str(feature_dict[node_list[0]]))
    ...
    node_list = list(reindexed_graph.nodes())
    ...
    # print("Features of first re-indexed node:")
    ...
    # print("feature_keys: " + str(feature_keys))
    # print("feature_keys length: " + str(len(feature_keys)))
...
class GraphSAGEAutoencoder(nn.Module):
    ...
    def generate_embedding(self, node, TRAIN=False):
        ...
        embed_elapsed_time = time.time() - embed_start_time
        print(f"aggregated neighbors in {embed_elapsed_time} seconds.")
        ...
        embedding_tensor[NODELEN] = 0.0 # these are zeroing out the wrong features
...
def run_group_formation_module(db_credentials, start_time, end_time):
    ...
    # networkx community grouping algorithms:
    ...
    # create new graph from adjacency (similarity) matrix
