...
"""
Scenario:
Let's say we have a small IP network where some IPs communicate with others. 
Each IP address is a node in our graph. The communication between two IPs is 
represented as an edge. Each IP also has a set of events (e.g., login attempts, data uploads, etc.), 
which are our node features.

Graph:
Nodes: IP Addresses
Edges: Communication between IPs
Node Features: Vector of events (e.g., number of login attempts, number of data uploads, etc.)

Goal:
We want to detect IP addresses (nodes) with correlated event patterns.

Step-by-step:
Data Creation: We'll create a dummy graph with IP addresses, their communication patterns, and event features.
GraphSAGE Implementation: We'll use the GraphSAGE algorithm to generate embeddings for each IP.
Correlation Detection: We'll measure the similarity between embeddings to detect correlated event patterns.
Let's start by setting up our dummy data:
"""
...
# Adjacency list representation of graph (communication patterns)
adj_list = {
...
# For simplicity, let's use 2D vectors: [number of login attempts, number of data uploads]
features = {
...
ip_addresses, adj_list, features
...
(['192.168.0.1', '192.168.0.2', '192.168.0.3', '192.168.0.4', '192.168.0.5'],
 {'192.168.0.1': ['192.168.0.2', '192.168.0.3'],
  ...
  '192.168.0.4': ['192.168.0.2'],
  '192.168.0.5': ['192.168.0.3']},
 {'192.168.0.1': array([ 5, 10]),
  '192.168.0.2': array([6, 9]),
  '192.168.0.3': array([ 4, 11]),
  '192.168.0.4': array([ 5, 10]),
  '192.168.0.5': array([6, 9])})

We have set up a toy network:

IP Addresses: ['192.168.0.1', '192.168.0.2', '192.168.0.3', '192.168.0.4', '192.168.0.5']

Communication Patterns (represented as an adjacency list):

'192.168.0.1' communicates with '192.168.0.2' and '192.168.0.3'
'192.168.0.2' communicates with '192.168.0.1' and '192.168.0.4'
... and so on.
Event Features: For each IP, we have a 2D feature vector:

First value represents the number of login attempts.
Second value represents the number of data uploads.
For example:

'192.168.0.1' has 5 login attempts and 10 data uploads.
'192.168.0.2' has 6 login attempts and 9 data uploads.
... and so on.
Next, we'll implement the GraphSAGE algorithm to generate embeddings for each IP. We'll use 
the mean aggregator as explained before.

"""
loaded_graphs, labels = dgl.load_graphs('C:/Users/cstanek/Documents/CooperStandard/tutorials/EUAIOps/sample_data_directed_edges.bin')
...
# Now, `nodes_data_list` contains dictionaries for each node with its features
...
"""
RESULT
{'192.168.0.1': array([ 5.  , 10.  ,  5.25,  9.75]),
 '192.168.0.2': array([6. , 9. , 5.5, 9.5]),
 '192.168.0.3': array([ 4. , 11. ,  4.5, 10.5]),
 '192.168.0.4': array([ 5., 10.,  5., 10.]),
 '192.168.0.5': array([6. , 9. , 5.5, 9.5])}
We have generated embeddings for each IP address using GraphSAGE with a mean aggregator:

'192.168.0.1': [ 5. , 10. , 5.25, 9.75]
'192.168.0.2': [6. , 9. , 5.5, 9.5]
... and so on.
Each embedding consists of two parts:

The original node features (e.g., [5., 10.] for '192.168.0.1').
The aggregated features from its neighbors (e.g., [5.25, 9.75] for '192.168.0.1').
...
"""
RESULT
{('192.168.0.2', '192.168.0.5'): 1.0,
 ('192.168.0.1', '192.168.0.4'): 0.9997601933564324,
 ('192.168.0.1', '192.168.0.2'): 0.9958365999273585,
 ('192.168.0.1', '192.168.0.5'): 0.9958365999273585,
 ('192.168.0.3', '192.168.0.4'): 0.9957385837170336,
 ('192.168.0.2', '192.168.0.4'): 0.9951990015225994,
 ('192.168.0.4', '192.168.0.5'): 0.9951990015225994,
 ('192.168.0.1', '192.168.0.3'): 0.9946741417428377,
 ('192.168.0.2', '192.168.0.3'): 0.9819322028323411,
 ('192.168.0.3', '192.168.0.5'): 0.9819322028323411}
"""

"""
Here's the pairwise cosine similarity between the embeddings of the IP addresses:

The pair ('192.168.0.2', '192.168.0.5') has the highest similarity of 1.0, indicating they have 
highly correlated event patterns.
The pair ('192.168.0.1', '192.168.0.4') has a similarity of approximately 0.9998, which is also 
very high, suggesting another set of IPs with correlated activities.
... and so on.

From this data, we can infer the following:

IP addresses '192.168.0.2' and '192.168.0.5' have very similar (or identical) event patterns.
IP addresses '192.168.0.1' and '192.168.0.4' also exhibit very similar event patterns.
You can set a threshold on similarity values to determine which pairs are considered to have "correlated" event patterns.
